    ...
    *,
    ...
    if official_warnings:
        return _risk_result(
            ...
            summary=(
                "A current official source warns about activity associated with this displayed number."
            ),
            reason_codes=_reason_codes(official_warnings),
            evidence=official_warnings,
            action_code="avoid_and_verify",
            action_message=(
                "Do not act on the call; contact the claimed organisation through a trusted channel."
            ),
        )
    blocking_gaps = [
        item
    ...
    if blocking_gaps:
        return _insufficient_result(
            sorted({str(item["code"]) for item in blocking_gaps})
        )
    eligible_signals = [
        item
        for item in evidence
    ...
    if corroborated_reason is not None:
        ...
        return _risk_result(
            ...
            summary=(
                "Multiple independent eligible sources report a consistent harmful-activity pattern."
            ),
    ...
    if risk_checks and all(item.get("status") == "no_match" for item in risk_checks):
        return {
            ...
            "summary": (
                "Current eligible risk sources returned no match; this is not proof that the number is safe."
            ),
    ...
    return _insufficient_result(insufficient_reasons)
