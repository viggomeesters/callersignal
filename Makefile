.PHONY: check test lint validate-go check-docs check-assets check-safety build-acm-catalog build-fcc-catalog

check:
	@bash scripts/check.sh

test:
	@uv run pytest -q

lint:
	@uv run ruff check tests scripts/validate_repository.py

validate-go:
	@bash scripts/validate-go.sh

check-docs:
	@bash scripts/check-docs.sh

check-assets:
	@bash scripts/check-assets.sh

check-safety:
	@bash scripts/check-public-safety.sh

build-acm-catalog:
	@uv run python scripts/build_acm_catalog.py --json

build-fcc-catalog:
	@uv run python scripts/build_fcc_catalog.py --json
