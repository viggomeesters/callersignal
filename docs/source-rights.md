...
One caller-reputation runtime is enabled: the manifest-bounded public-domain FCC aggregate. The source index contains sixteen services: that official data API, eleven permission or product-fit candidates, and four advertised commercial licensing routes. An advertised product is not a CallerSignal licence. With the checked-in registry and no approved partner credential, the commercial activation engine creates zero third-party reputation adapters and performs zero report-page or partner-API requests.

### First Dutch licensed-source candidate

[`sources/tellows-nl-acquisition.json`](../sources/tellows-nl-acquisition.json) selects the tellows Live API partnership route for contract review as the first additional Dutch risk-source candidate. This is a selection for due diligence, not authorization or activation. The operator maintains a Dutch-language service and advertises country-filtered Live and Scorelist API integrations. Hiya is deferred because its public Number Reputation documentation is oriented to registered business numbers and requires authority to represent third-party businesses; that is not the first fit for arbitrary inbound-number checks.

The proposed CallerSignal boundary is a transient E.164 point lookup that admits only an opaque provider record identifier, current score, category, and observation time. Names, comments, authors, location, postal code, search volume, bulk number lists, raw responses, and provider “safe” labels are forbidden. The provider's published caller-identification explanation describes scores 7–9 as its most untrustworthy range, so those are the only candidates for a mapped negative observation; the executed agreement must still confirm current API semantics. Even then, the result remains one unverified source observation and cannot independently create `elevated_signals` or `official_warning`. Lower scores and no-match responses become no positive safety evidence.

The machine contract keeps the source absent from the enabled registry and requires zero runtime requests. Activation requires an executed agreement that explicitly covers extraction, caching, derived labels, public display through web/API/CLI/MCP, Netherlands territory, attribution, privacy and data-processing roles, correction and deletion, provenance, audit, termination and purge, credentials, exact fields, freshness, rate limits, outage, and schema drift. A later activation task must carry named human commercial and privacy approval; repository tests cannot manufacture that authority.
...
The read model authenticates catalogue metadata and the keyed index before lookup. It may return only an unverified `nuisance` or `robocall` aggregate at fixed low evidence confidence. Complaint count cannot increase confidence, replace source diversity, identify a caller, or create `elevated_signals` or `official_warning`. A catalogue no-match is an absence of source data, not a safety claim.
