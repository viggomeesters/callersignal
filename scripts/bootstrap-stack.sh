...
if [ -n "${GO_STACK_REF:-}" ]; then
  echo "GO_STACK_REF cannot override .go/project.json; update stack_ref instead" >&2
  exit 5
fi
...
if [ ! -d "$GO_STACK/.git" ]; then
  ...
  if ! runtime_matches "$GO_STACK"; then
    if allow_development_checkout; then
      echo "warning: development override accepts unpinned GO_STACK; expected $STACK_REF" >&2
    else
  ...
  if ! managed_origin_matches; then
    echo "managed go-workflow-stack cache origin does not match GO_STACK_REMOTE" >&2
    exit 6
  fi
  if [ -n "$(git -C "$GO_STACK" status --porcelain)" ]; then
    echo "go-workflow-stack checkout is dirty; clean it before bootstrap updates" >&2
    exit 3
  fi
  if ! runtime_matches "$GO_STACK"; then
...
if ! runtime_matches "$GO_STACK"; then
  if allow_development_checkout; then
    echo "warning: development override remains active for $GO_STACK" >&2
  else
    echo "go-workflow-stack runtime mismatch: expected $STACK_REF" >&2
    exit 4
  fi
fi
